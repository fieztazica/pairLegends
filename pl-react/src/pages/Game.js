    ...
    switch (value) {
      ...
      default:
        return (
          <GameMode
            colNumValue={colNum}
            colNumOnChange={colNumChange}
            champsValue={champs}
            champsOnChange={champsChange}
            playOnClick={handleOnPlay}
            timerValue={timer}
            timerOnChange={timerChange}
          />
        );
    }
  };
        ...
        <Box alignItems="center" display="flex" flexDirection="column">
          ...
          {status === "play" && (
            <Typography>
              {`${
                tiles && champ1 && champ2
                  ? `Champ1 (${champ1.x}.${champ1.y}): ${
                      Champions[`${tiles[champ1.x][champ1.y]}`]
                    } Champ2 (${champ2.x}.${champ2.y}): ${
                      Champions[`${tiles[champ2.x][champ2.y]}`]
                    }`
                  : "Tiles empty"
              }`}
            </Typography>
